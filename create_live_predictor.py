"""Create live predictor module and update API."""
...
# File 1: live_predictor.py content
live_predictor_code = """
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    for i in range(15):
        ...
        print(f"  {i+1:2d}: risk={result['risk_score']:.3f} ({result['risk_level']}) buffer={result['buffer_size']}")
...
# Create live_predictor.py
backend_dir = Path("backend")
with open(backend_dir / "live_predictor.py", "w") as f:
    f.write(live_predictor_code.strip())
print("Created backend/live_predictor.py")

# Add endpoint to api.py if not exists
api_file = backend_dir / "api.py"
with open(api_file, "r") as f:
    api_content = f.read()

if "/api/predict/live" not in api_content:
    with open(api_file, "a") as f:
        f.write(api_endpoint_code)
    print("Added live endpoint to backend/api.py")
else:
    print("Live endpoint already exists")

# Copy to clean export
clean_dir = Path("te_predictive_maintenance_CLEAN")
if clean_dir.exists():
    shutil.copy(backend_dir / "live_predictor.py", clean_dir / "backend" / "live_predictor.py")
    shutil.copy(backend_dir / "api.py", clean_dir / "backend" / "api.py")
    print("Copied files to clean export")
