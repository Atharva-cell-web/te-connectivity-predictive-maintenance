...
def _load_feb_results():
    ...
    _feb_cols = [
        "timestamp", "Injection_pressure", "Cycle_time", "is_scrap_actual"
    ]
    try:
    ...
    if "scrap_probability" in feb.columns:
        feb = feb.drop(columns=["scrap_probability"])
    return feb
...
def _load_machine_pivot(machine_norm: str):
    ...
    # FIX: Forward fill asynchronous sensors to prevent missing data gaps
    pivot = pivot.ffill().bfill()

    # COMPUTE V2 ROLLING FEATURES
    ...
    base_sensors = [
        "Cushion", "Cycle_time", "Cyl_tmp_z1", "Cyl_tmp_z2", "Cyl_tmp_z3", "Cyl_tmp_z4",
        "Cyl_tmp_z5", "Cyl_tmp_z6", "Cyl_tmp_z7", "Cyl_tmp_z8", "Dosage_time",
        "Ejector_fix_deviation_torque", "Extruder_start_position", "Extruder_torque",
        "Injection_pressure", "Injection_time", "Machine_status", "Peak_pressure_position",
        "Peak_pressure_time", "Scrap_counter", "Shot_counter", "Shot_size",
        "Switch_position", "Switch_pressure", "Time_on_machine"
    ]

    pivot = pivot.set_index("timestamp").sort_index()

    for col in base_sensors:
        if col in pivot.columns:
            for w_val, w_str in [("5min", "5m"), ("15min", "15m"), ("30min", "30m")]:
                roller = pivot[col].rolling(w_val)
                pivot[f"{col}__mean_{w_str}"] = roller.mean()
                pivot[f"{col}__std_{w_str}"] = roller.std().fillna(0.0)
                pivot[f"{col}__min_{w_str}"] = roller.min()
                pivot[f"{col}__max_{w_str}"] = roller.max()
                pivot[f"{col}__last_{w_str}"] = pivot[col].ffill()

    pivot = pivot.reset_index()

    # 3. FIX TOOL ID EXTRACTION
    tool_id = "UNKNOWN"
    if machine_definition != "UNKNOWN":
        digits = re.findall(r'\d+', str(machine_definition).split('-')[-1])
        if digits:
            tool_id = digits[-1]
    machine_norm_clean = machine_norm.upper().replace("-", "")

    pivot["tool_id_encoded"] = encodings.get("tool_id", {}).get(tool_id, 0.0)
    pivot["machine_id_encoded"] = encodings.get("machine_id", {}).get(machine_norm_clean, 0.0)
    pivot["part_number_encoded"] = 0.0

    # Fill missing model columns with 0.0 to match the V2 training baseline
...
def _build_machine_feb_history(machine_norm: str):
    ...
    # --- STRICT NUMPY ALIGNMENT FOR V2 MODEL ---
    model, model_features = _load_control_model_and_features()
    # Mathematically guarantee a shape of (N, 378)
    X_all = np.zeros((len(history), len(model_features)), dtype=np.float32)
    for i, col in enumerate(model_features):
        X_all[:, i] = history.get(col, 0.0).fillna(0.0).astype(np.float32).values
    if hasattr(model, "predict_proba"):
        history["scrap_probability"] = model.predict_proba(X_all)[:, 1]
    else:
        history["scrap_probability"] = model.predict(X_all)

    history["scrap_probability"] = history["scrap_probability"].clip(0, 1)
    ...
    # Extract Tool ID again for the payload output
    tool_id = "UNKNOWN"
    if machine_definition != "UNKNOWN":
        digits = re.findall(r'\d+', str(machine_definition).split('-')[-1])
        if digits:
...
def _generate_future_horizon(past_window: pd.DataFrame, future_minutes: int, current_safe_limits: dict = None):
    ...
    # --- STRICT NUMPY ALIGNMENT FOR FUTURE HORIZON ---
    X_future = np.zeros((len(future), len(model_features)), dtype=np.float32)
    for i, col in enumerate(model_features):
        X_future[:, i] = future.get(col, 0.0).fillna(0.0).astype(np.float32).values
    if hasattr(model, "predict_proba"):
