"""
Step 4c: Extract and encode part_number and tool_number features.
These categorical features help the model learn tool-specific scrap patterns.
"""
...
def extract_tool_from_definition(machine_definition: str) -> str:
    """Extract tool ID from machine_definition string."""
    if pd.isna(machine_definition) or not machine_definition:
        return "UNKNOWN"
    # Pattern: typically ends with tool ID like "M-471-ABC123"
...
def load_machine_context() -> pd.DataFrame:
    """Load part_number and tool_number from all machine parquet files."""
    print("[INFO] Loading machine context (part/tool) from parquet files...")
    
    context_records = []
    machine_files = list(MACHINE_DATA_DIR.glob("M*_TEST.parquet"))
    
    print(f"[INFO] Found {len(machine_files)} machine files")
